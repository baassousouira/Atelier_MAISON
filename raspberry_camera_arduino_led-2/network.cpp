...
// -------------------------------------------------------------------
// send_heartbeat : signale au serveur central que ce Raspberry est en vie,
// avec l'état de chacun de ses composants
// -------------------------------------------------------------------
bool send_heartbeat(bool a_un_servo, const std::vector<EtatComposant> &composants) {
    CURL *curl = curl_easy_init();
    if (!curl) return false;

    // Construction du tableau JSON "components": [...]
    std::string composants_json = "[";
    for (size_t i = 0; i < composants.size(); ++i) {
        const EtatComposant &c = composants[i];
        composants_json += "{\"name\":\"" + c.nom + "\",\"enabled\":" + (c.actif ? "true" : "false");
        if (c.a_une_valeur) {
            composants_json += ",\"value\":" + std::to_string(c.valeur);
        }
        composants_json += "}";
        if (i + 1 < composants.size()) composants_json += ",";
    }
    composants_json += "]";

    std::string json_payload =
        "{\"has_servo\":" + std::string(a_un_servo ? "true" : "false") +
        ",\"components\":" + composants_json + "}";

    std::string url = std::string(SERVER_BASE_URL) + "/api/controleurs/heartbeat";
    std::string response_body;
    ...
    if (!ok) {
        std::cerr << "network: échec envoi heartbeat (" << curl_easy_strerror(res) << ")" << std::endl;
    }
...
// -------------------------------------------------------------------
// lire_etat_armed : demande au serveur si la surveillance est armée
// -------------------------------------------------------------------
bool lire_etat_armed(bool &armed_out) {
    CURL *curl = curl_easy_init();
    if (!curl) return false;

    // Route confirmée dans le vrai serveur central : /api/system (pas /api/armed)
    std::string url = std::string(SERVER_BASE_URL) + "/api/system";
    std::string response_body;
    ...
    CURLcode res = curl_easy_perform(curl);
    if (res != CURLE_OK) {
        std::cerr << "network: échec lecture armed (" << curl_easy_strerror(res) << ")" << std::endl;
        return false;
    }

    std::string valeur = extract_json_field(response_body, "armed");
    armed_out = (valeur == "true");
    return true;
}
