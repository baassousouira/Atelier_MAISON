...
inline void camera_definir_activee(bool activee) {
    ...
    // Lorsqu'on éteint la caméra, on efface immédiatement la dernière
    // image exposée afin que le serveur HTTP ne continue pas à montrer
    // une ancienne frame figée.
    if (!activee) {
        std::lock_guard<std::mutex> verrou(camera_mutex_image);
        camera_derniere_image_jpeg_interne.clear();
    }
